...
export const Route = createRootRouteWithContext<{
  ...
  head: () => ({
    meta: [
      {
        title: 'Angular Start SPA Mode',
      },
      {
        name: 'description',
        content:
          'An Angular Start SPA mode example wired through the current experimental adapter.',
      },
    ],
    scripts: [
      {
...
@Component({
  ...
  template: `
    <div class="mx-auto flex min-h-screen max-w-5xl flex-col gap-6 px-4 py-6">
      <header
        class="space-y-3 rounded-2xl border bg-white/80 p-5 shadow-sm dark:bg-gray-950/70"
      >
        <div class="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            >
              Angular Start SPA Mode
            </p>
            <h1 class="text-2xl font-semibold">
              Static shell with client routing
            </h1>
          </div>
          <nav class="flex flex-wrap gap-2 text-sm">
            <a
              [link]="{
                to: '/',
                activeProps: { class: 'bg-teal-600 text-white' },
        ...
        <p class="max-w-3xl text-sm text-gray-600 dark:text-gray-300">
          This example enables Start SPA mode so the build emits a reusable
          shell while route content is rendered by Angular on the client.
        </p>
      </header>

      <main
        class="rounded-2xl border bg-white/80 p-5 shadow-sm dark:bg-gray-950/70"
      >
        <outlet />
      </main>
