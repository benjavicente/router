...
This example showcases **file-based route generation** for Angular using `@benjavicente/angular-router-experimental` and `@benjavicente/router-plugin` with Vite, aligned with the React basic-file-based example.

## What's demonstrated

- **Target: Angular** – The router generator is configured with `target: 'angular'` so route files use Angular standalone components and `@benjavicente/angular-router-experimental'`.
