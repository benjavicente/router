...
The same applies to marker-protected files (`import '@benjavicente/react-start/server-only'`). If a marked file is re-exported through a barrel but never consumed by client code, the build correctly suppresses the violation while dev may still warn.
