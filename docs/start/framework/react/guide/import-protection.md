...
- **Specifier patterns** match on the raw import string. By default, `@benjavicente/react-start/server` is denied in the client environment.
...
- The specifier `@benjavicente/react-start/server`
...
The same applies to marker-protected files (`import '@benjavicente/react-start/server-only'`). If a marked file is re-exported through a mixed barrel but never consumed by client code, production may suppress the warning after tree-shaking, but the better fix is still to avoid exposing that server-only edge to client-reachable code at all.
